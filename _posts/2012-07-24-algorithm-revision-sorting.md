...
排序问题(sorting)是理论计算机研究当中最基本也是研究最为深入的一个部分. 下面几个有趣的事实体现了排序在信息科学里的重要性:
 ...
 + 超过25%的cpu时间花在了排序上
 + 排序出现在各种各样的场景中, 哪怕只是打开手机上的联系人名单
 + 排序是很多其他算法的前置条件, 比如二分搜索(binary search)
 + 研究排序的过程中提出的很多算法设计方法被更广泛的应用到了其他领域, 比如分治法(divide-conquer-combine)

### 排序算法的模型 ###

排序的方法有很多, 有些需要`\(O(n^2)\)`时间(插入排序, 冒泡排序), 有些能够在`\(O(nlgn)\)`时间内完成n个元素的排序(快排的平均情况, 合并排序和堆排序), 而另外一些算法在特定的情况下能够在线性时间内完成(计数排序, 桶排序等). 这些算法的复杂度的区别来源于算法本身的模型. 一种常见的排序模型是基于比较的排序, 插入排序, 快速排序等等均属于这个范畴. 在这些排序的结果当中, 各个元素的次序是基于对输入元素之间的比较来决定的.
...
插入排序是一种简单的基于比较的排序算法, 包含`(n-1)`次插入操作. 插入排序的思路是在待排序的元素区间中维护一个已经排序的子区间, 然后每次把这个子区间外的一个元素插入到子区间中的正确的位置上, 从而把已排序的区间的大小增加了1. 通常我们从第二个元素开始(第一个元素自己是已经排好序的了), 不断执行插入操作, 直到第`(n-1)`个元素.

插入排序在一般情况下是一个`(O(n^2))`的算法. 但是如果待排序区间是已经排序或者接近已排序的状态时, 插入排序可以加速到近似线性时间. 在有的实现里, 合并排序或者是快拍这种递归排序算法, 递归到比较小的区间上时, 会转到使用插入排序.
...
理论上说, 一个最佳的排序算法需要满足下列条件:

 1. 稳定的, 即排序过程中保留等值元素的相对位置. 这样减少不必要的数据读写
 2. 原地排序, 只需要`\(O(1)\)`的存储空间
 3. 最坏情况`\(O(nlg(n))\)`次比较(即运算时间)
 4. 最坏情况`\(O(n)\)`次元素交换
 5. 当输入序列接近有序时(nearly-sorted)或者有大量重复元素时, 算法应能够加速到`\(O(n)\)`

没有一种已知的排序算法同时满足这些要求, 所以在它们的选择上要根据具体应用的需要而定.
